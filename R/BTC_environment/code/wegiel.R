...
energy_by_fuel <- cbind(c("Olej","Gaz naturalny", "Wêgiel", "Energia j¹drowa", "Elektrownia wodna","Odnawialne"),energy_by_fuel)
...
p1 <- energy_by_fuel %>% 
  ...
  labs(y = "Procent energii", title = "Paliwa wykorzystywane do produkcji energii do kopania BTC", fill="ród³a")+
  coord_flip() 
p1


### wartosci trzeba jeszcze podzielic przez 0,4 poniewaz takie mniej wiecej jest efficiency
do_liczenia = energy_by_fuel %>%
  mutate(Energy = Energy) # jest w TWh

#wegiel 70.67 TWh =8 680 751 ton wegla // 8.7 mln ton ==
# naturalny gaz
  
# 800 pounds :
wagi=c(800,2.4,1.5,186,0.64,1680)*0.45359237 # w kg     coal,sulfur dioxide,nitrogen oxides,coal ash,uranium and thorium,carbon dioxide 
wagi = wagi/1000  #w tonach
wagi= wagi * 1/0.3628738960
wagi
